...
class NineTileApp(ui.View):
    ...
    def _build_ui(self):
        ...
        # ここでボタンを1つだけ作成
        by = gy + gh + 36
        self.btn_main = ui.Button(title='START', frame=(20, by, screen_w, 320), action=self._on_main_button)
        self.btn_main.bg_color = '#4A4A4A'
        self.btn_main.tint_color = 'white'
        self.btn_main.border_width = 0.5
        self.btn_main.corner_radius = 6
        self.btn_main.width = 300
        self.btn_main.height = 200
        self.add_subview(self.btn_main)

    def _on_main_button(self, sender):
        if self.state == 'IDLE':
            # IDLE → RUNNING
            self.btn_main.title = 'STOP'
            ...
            self.lbl_feedback.text = f'⏸ {el:.2f}s recorded – tap to NEXT →'
            self.lbl_feedback.text_color = 'orange'
            self.btn_main.title = 'NEXT'
            self.state = 'PAUSED'

        elif self.state == 'PAUSED':
            # PAUSED → 次のパターン or 終了
            if self.trial >= 30:
                self._finish()
                return
            self._next_pattern()
    ...
    def _check_answer(self):
        ...
        if correct:
            ...
            play_effect(SE_CONFLICT)
        else:
            play_effect(SE_INCORRECT)
